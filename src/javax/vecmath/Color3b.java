...
/**
 * A three-byte color value represented by byte x, y, and z values. The x, y, and z values represent
 * the red, green, and blue values, respectively.
 * <p>
 * Note that Java defines a byte as a signed integer in the range [-128, 127]. However, colors are
 * more typically represented by values in the range [0, 255]. Java 3D recognizes this and for color
 * treats the bytes as if the range were [0, 255]---in other words, as if the bytes were unsigned.
 * <p>
 * Java 3D assumes that a linear (gamma-corrected) visual is used for all colors.
 *
 */
public class Color3b extends Tuple3b<Color3b> implements java.io.Serializable {
