...
class VecMathUtil {
	 ...
	 * mapping -0.0 to 0.0. This method is identical to Float.floatToIntBits(float)
	 * except that an integer value of 0 is returned for a floating-point value of
	 * -0.0f. This is done for the purpose of computing a hash code that satisfies
	 * the contract of hashCode() and equals(). The equals() method in each vecmath
	 * class does a pair-wise "==" test on each floating-point field in the class
	 * (e.g., x, y, and z for a Tuple3f). Since 0.0f&nbsp;==&nbsp;-0.0f returns
	 * true, we must also return the same hash code for two objects, one of which
	 * has a field with a value of -0.0f and the other of which has a cooresponding
	 * field with a value of 0.0f.
	 *
	 ...
	 * returned for a floating-point value of -0.0. This is done for the purpose of
