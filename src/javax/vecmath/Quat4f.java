...
public class Quat4f<T extends Quat4f> extends Tuple4f<T> implements java.io.Serializable {
 ...
 public Quat4f(Tuple3f t1, float w) {
  super(t1, w);
 }

 /**
  * Sets the value of this quaternion to the quaternion product of quaternions q1 and q2 (this = q1
  * * q2). Note that this is safe for aliasing (e.g. this can be q1 or q2).
  *
  * @param <T>
 ...
 /**
  * Performs a great circle interpolation between this quaternion and the quaternion parameter and
 ...
 /**
  * Transform the 3 tuple t1 by this quaternion and store the result back into t1. t1 = q*t1*q'
  *
  * @param <S>
  * @param t1 the point to transform
  * @return t1 for chaining
  */
 public <S extends Tuple3f> S transform(S t1) {
  final Quat4f q = new Quat4f(this).mul(new Quat4f(t1.x, t1.y, t1.z, 0)).mul(new Quat4f(this)
   .conjugate());
  t1.set(q.x, q.y, q.z);
  return (S) t1;
 }
 ...
 /**
  * Transform the 3 tuple t1 by the conjugate of this quaternion and store the result back into t1.
  * t1 = q'*t1*q
  *
  * @param <S>
  * @param t1 the point to transform
  ...
  *
  */
 public <S extends Tuple3f> S conjugateTransform(S t1) {
  final Quat4f q = new Quat4f(this).conjugate().mul(new Quat4f(t1.x, t1.y, t1.z, 0)).mul(this);
  t1.set(q.x, q.y, q.z);
  return (S) t1;
 }
}
